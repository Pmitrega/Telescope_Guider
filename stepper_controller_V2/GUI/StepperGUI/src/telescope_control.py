...
class TelescopeController:
    def __init__(self, set_ra_speed_func: Callable[[int], None], set_dec_speed_func: Callable[[int], None]):
        ...
        self.go_to_loc = starCentroid(0, 1280 / 2, 960 / 2)
    ...
    def drawArrowsOnImage(self, Image: np.ndarray):
        ...
        dec_vect = self.telescope_dec_vect
        colored = cv2.cvtColor(Image.astype(np.uint8), cv2.COLOR_GRAY2RGB)
        if not (math.isnan(ra_vect[0]) or math.isnan(ra_vect[1]) or math.isnan(dec_vect[0]) or math.isnan(dec_vect[1])):
            cv2.arrowedLine(colored, (100, 100), (100 + int(ra_vect[0] * 50), 100 + int(ra_vect[1] * 50)),
                            (255, 0, 0), 3)
            cv2.arrowedLine(colored, (100, 100), (100 + int(dec_vect[0] * 50), 100 + int(dec_vect[1] * 50)),
                            (0, 255, 0), 3)

        sky_ra_vect = self.sky_ra_vect
        sky_dec_vect = self.sky_dec_vect
        cv2.arrowedLine(colored, (1000, 100), (1000 + int(sky_ra_vect[0] * 50), 100 + int(sky_ra_vect[1] * 50)),
                        (0, 0, 255), 3)
        cv2.arrowedLine(colored, (1000, 100), (1000 + int(sky_dec_vect[0] * 50), 100 + int(sky_dec_vect[1] * 50)),
                        (0, 255, 255), 3)
        return colored

    def showTrackedStar(self, Image: np.ndarray):
        if self.reference_star_current is not None:
            cv2.circle(Image, (int(self.reference_star_current.x_cent), int(self.reference_star_current.y_cent)),
                       int(math.sqrt(self.reference_star_current.brightness / 2)), (0, 0, 255), 3)

    def showAllStars(self, Image: np.ndarray):
        for star in self.last_star_centroids:
    ...
    def findStars(self):
        ...
        bin_im, star_centroids = star_detection.segmentation(I2Bin.astype(np.uint8), -15)
